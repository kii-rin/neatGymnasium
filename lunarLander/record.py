...
def load_genome(local_dir):
    ...
    checkpoint = max(checkpoints, key=lambda p: int(os.path.basename(p).split("-")[1]))
    print("Loading", checkpoint)
    pop = neat.Checkpointer.restore_checkpoint(checkpoint)
    best = max(
        [g for g in pop.population.values() if g.fitness is not None],
...
def main():
    ...
    env = RecordVideo(env, video_folder=output_dir, name_prefix="lander")
